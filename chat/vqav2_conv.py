...
from prompts.vqav2_prompt import *
import json
import os
...
class VQAV2ConversationTwoAgent(ExtendDescriptionTwoAgent):
    def __init__(self, img, vqa_model, model, question, label, data_id, prompt_setting='v1a', caption=None, temp_gpt=0, description_extend = False, objects_prompt_type = 'question', temp_name = 'temp'):
        if type(self) == VQAV2ConversationTwoAgent:
    ...
    def chatting(self, max_n_rounds, print_mode):
        ...
        for round_i in tqdm(range(max_n_rounds), desc='Chat Rounds', disable=print_mode != 'bar'):
            ...
            # Input sub-questions and sub-answers into inferencer LLM.
            ...
            while try_num < max_try:
                ...
                if not need_rerun:
                    ...
                    if try_num == max_try:
                        ...
                        print(f'Parsing failed, Time {try_num} of Rerun LLM Decision for data {self.data_id}.')
