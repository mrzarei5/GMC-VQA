from data.okvqaTools.okvqa import OKVQA
import random
...
class OKVQASampler():
    ...
    def __init__(self, dataDir , dataSubType ='val', taskType ='OpenEnded', dataType ='mscoco', data_num = 100):
        random.seed(10)
        ...
        self.ann_file = '%s/Annotations/%s_%s_annotations.json'%(dataDir, dataType, self.data_sub_type)
        self.ques_file = '%s/Questions/%s_%s_%s_questions.json'%(dataDir, taskType, dataType, self.data_sub_type)
        self.img_dir = '%s/Images/%s/' %(dataDir, self.data_sub_type)

        self.vqa=OKVQA(self.ann_file, self.ques_file)
        ...
        for annot_id in ids:
            ...
            answer = self.vqa.detAns(annotation)
            if answer:
        ...
        for annot_id in self._ids:
            ...
            annotation_this['answer_label'] = self.vqa.getAns(annotation['question_id'])
